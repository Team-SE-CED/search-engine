...
import { usePapersByAbstract } from "~/composables/use-paper-abstract";
const { getPapersByAbstract } = usePapersByAbstract();
...
const researchAbstract = ref<Paper[]>([]);
...
// Functions
async function fetchPaperByAbstract(query: string) {
  try {
    researchAbstract.value = await getPapersByAbstract(query);
  } catch (error) {
    console.error("Error fetching papers by abstract:", error);
  }
}
...
const handleSubmit = async () => {
  if (searchQuery.value.trim()) {
    await fetchPaperByAbstract(searchQuery.value); // Fetch papers by abstract
